...
export function normalizeWorkoutPlan(
  ...
  return {
    ...
    exercises: defaultPlan.exercises.map((defaultExercise, index) => {
      const stored =
        plan.exercises?.find(
          (exercise) => exercise?.id === defaultExercise.id,
      ...
      return {
        ...
        name: stored.name,
        targetSets: clampInt(
          stored.targetSets ?? defaultExercise.targetSets,
          1,
        ),
        targetReps: clampInt(
          stored.targetReps ?? defaultExercise.targetReps ?? 1,
          1,
        ),
        restSeconds: clampInt(
          stored.restSeconds ?? defaultExercise.restSeconds,
          15,
        ),
